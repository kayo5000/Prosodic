...
- **1a.** Move the already-isolated, zero-framework-dependency modules into `domain/` first: `song_context.py`, `final_result_converter.py`, `prosodic_config.py`. Lowest risk — each already has zero Flask/DB/vendor imports, so this is close to a pure file move plus import-path updates, and it proves the layering pattern works end-to-end (including how tests reference these modules) before touching anything riskier.
- **1b.** Move the live-pipeline engines (`phoneme_engine.py`, `syllable_engine.py`, `rhyme_detection_engine.py`, `motif_engine.py`, `density_engine.py`, `pocket_engine.py`, `phrase_container_engine.py`, `perceptual_family_engine.py`, `pattern_reader_engine.py`, `semantics_engine.py`, `thesaurus_engine.py`, `concreteness_engine.py`, `feedback_engine.py`, `suggestion_engine.py`, and the dormant-but-real engines from `docs/ARCHITECTURE.md`) into `domain/` (or a `domain/engines/` subpackage — decide during 1a based on what reads cleanest with only 3 files in place). This is the bulk of the work — ~14+ interdependent files, done in small, individually-verified batches, not one giant commit. `suggestion_engine.py`'s known encoder+decoder-both smell (`docs/ARCHITECTURE.md` §3) is a real candidate to actually fix *while* it's already being moved — same file touched either way, cheaper to do both at once than revisit it a second time. Flagged as a decision to make in the moment, not pre-committed to here.
