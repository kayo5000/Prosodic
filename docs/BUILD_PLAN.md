...
- **1d.** Reorganize how `api.py` calls into business logic so every route depends inward on `domain/`/`application/` and never constructs business logic inline. Given most routes already just parse input and delegate (confirmed for `/analyze` and `/suggest` during 1c), this is likely lighter than 1b — verify that against the remaining ~40 routes rather than assuming it generalizes.
